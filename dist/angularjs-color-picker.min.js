...
b.initConfig(),
// setup mouse events
a.on("mousedown",b.onMouseDown),a.on("mouseup",b.onMouseUp),a.on("mousemove",b.onMouseMove),a.on("keydown",b.onKeyDown),b.find(".color-picker-grid").on("click",b.onColorClick),b.find(".color-picker-hue").on("click",b.onHueClick),b.find(".color-picker-opacity").on("click",b.onOpacityClick)},b.onKeyDown=function(a){9===a.which&&b.$apply(function(){b.hide()})},b.onMouseDown=function(a){
// an element in this picker
b.find(a.target).length>0&&(b.isPicking=!0,
// mouse event on color grid
a.target.classList.contains("color-picker-grid-inner")||a.target.classList.contains("color-picker-picker")||a.target.parentNode.classList.contains("color-picker-picker")?(b.colorDown(a),b.$apply()):a.target.classList.contains("color-picker-hue")||a.target.parentNode.classList.contains("color-picker-hue")?(b.hueDown(a),b.$apply()):(a.target.classList.contains("color-picker-opacity")||a.target.parentNode.classList.contains("color-picker-opacity"))&&(b.opacityDown(a),b.$apply()))},b.onMouseUp=function(a){b.isPicking=!1,
// no current mouse events and not an element in the picker
b.colorMouse||b.hueMouse||b.opacityMouse||0!==b.find(a.target).length?b.colorMouse?(b.colorUp(a),b.$apply(),b.onChange(a)):b.hueMouse?(b.hueUp(a),b.$apply(),b.onChange(a)):b.opacityMouse&&(b.opacityUp(a),b.$apply(),b.onChange(a)):(b.log("Color Picker: Document Click Event"),b.$apply(function(){b.hide()}))},b.onMouseMove=function(a){
// mouse event on color grid
